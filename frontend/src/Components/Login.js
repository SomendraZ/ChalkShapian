    ...
    signInWithEmailAndPassword(auth, email, password)
      .then(async (res) => {
        ...
        if (userSnapshot.exists()) {
          ...
          const chalkName = userData.chalkName;
  ...
  const popUp = async () => {
    try {
      ...
      if (docSnapshot.exists()) {
        // user already exists, retrieve chalk name
        const existingUser = docSnapshot.data();
        const chalkName = existingUser.chalkName;

        // store chalk name and login status in session storage
        sessionStorage.setItem("LoggedIn", true);
        sessionStorage.setItem("chalkName", chalkName);
